...
export const WebSocketProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // 下のwsと後に出てくるsocketは同じものを参照している。wsのほうはコメントにする。 - kotani
  ...
  // データ長をここで定義する（角谷バージョンは100、鈴木バージョンは180） - kotani
  const dataLen = 60
  ...
  useEffect(() => {
    ...
    // ------------------------------------------------------------
    // 注：useEffectの中でStateを更新している - kotani
    // ------------------------------------------------------------
    loadInitialData();
    ...
    /* 勝手にリロードされると困る！- kotani (2025/10/23)
    // 30分ごとに自動的にページをリロード
  ...
  useEffect(() => {
    ...
    const connect = () => {
      try {
        ...
        if (ws) {                           // 既存の接続があれば閉じる - kotani (2025/10/23)
            ws.onclose = null;          // 再接続ループを防ぐ - kotani (2025/10/23)
            ws.close();
        }
        
        setConnectionStatus('connecting');
        retryCountRef.current++;
        const websocket = ws = new WebSocket("ws://localhost:8765");
        // setWs(websocket);
        ...
        websocket.onopen = () => {
          ...
          try {
            if (allDevicesData && allDevicesData.devices && allDevicesData.devices.length > 0) {
              if (ws && ws.readyState == WebSocket.OPEN) {    // 状態をチェック - kotani
                websocket.send(JSON.stringify(allDevicesData));
              } else {
                console.log("Cannot send message: allDevicesData");   // エラー・メッセージをコンソールに出力 - kotani
              }
            }
          } catch (error) {
          ...
          try {
            ...
            if (restData.join(":").includes("301,")) {
              ...
              if (ws && ws.readyState === WebSocket.OPEN) {    // 状態をチェック - kotani
                ...
                console.log("Cannot send message:", message);   // エラー・メッセージをコンソールに出力 - kotani
              }
            ...
            if (deviceData.trackstate && deviceData.num_current !== undefined) {
              ...
              /* 障害の切り分けのためトラッキング電流値の反映を止めておく - kotani (2026/03/06)
                ...
                Object.keys(deviceData.trackcurrent).forEach((circuit) => {
                  ...
                  if (trackCurrentData && trackCurrentData.length > 0) {
                    ...
                    if (existingIndex >= 0) {
                      ...
                      // 配列サイズを制限
                      if (existingCurrentData.length > dataLen) {
                        deviceData.current[circuitNum] = existingCurrentData.slice(-dataLen);
                      }
                    }
                  }
                });
            ...
            // ------------------------------------------------------------
            // デバイスタイプ(DesconDevice or CubicleDevice)に応じてデータを更新
            // ------------------------------------------------------------
                  ...
                  Object.entries(newValue).forEach(([circuit, circuitData]) => {
                    ...
                    if (circuitData && Array.isArray(circuitData) && circuitData.length > 0) {
                      ...
                      if (newData && Array.isArray(newData) && newData.length > 0) {
                        ...
                        if (existingData.length > dataLen) {
                          existingData = existingData.slice(-dataLen);
                        }
                      }
                    }
        ...
        websocket.onclose = (event) => {
          ...
          if (retryCountRef.current < maxRetries) {
            ...
            if (isComponentMounted) {                       // コンポーネントがマウントされている場合のみ再接続 - kotani (2025/10/23)
                setTimeout(() => {
    ...
    return () => {
      ...
      */
      isComponentMounted = false;                                                       // kotani (2025/10/23)
      if (ws) {                                                                                     // kotani (2025/10/23)
        ws.onclose = null;                                                                     // kotani (2025/10/23)
        ws.onerror = null;                                                                      // kotani (2025/10/23)
        ws.onmessage = null;                                                                // kotani (2025/10/23)
        ws.onopen = null;                                                                       // kotani (2025/10/23)
        ws.close();                                                                                 // kotani (2025/10/23)
      }
    };
  ...
  }, [allDevicesData]); // socketは余計では？ - kotani

  // ------------------------------------------------------------
  // DataRequestのデータを解析する関数
  // ------------------------------------------------------------

...
async def fetch_device_data(device, websocket, request_type="DataRequest"):
    ...
    hex_string_map = {
        "DataRequest": "44617461526571756573740d0a0d0a",  # DataRequest
        "TrackingRecordRequest": "547261636b696e675265636f7264526571756573740d0a0d0a"  # TrackingRecordRequest
    }
    
    while True:
        hex_string = hex_string_map.get(request_type)
        end_marker = b"170,"  # データの終端を示すマーカー

        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                # 無線接続やデバッグ中の装置のため、タイムアウトを延長
                timeout = 15.0  # 3.5秒 → 15秒に延長
                ...
                try:
                    ...
                    while True:
                        ...
                        if end_marker in buffer:
                                # 170,の後ろにある数値の最後までデータを取得するため、改行まで含める
                                end_index = buffer.find(end_marker)
                                rest_data = buffer[end_index:].split(b'\r\n', 1)
                                if len(rest_data) > 1:
                                    end_index += len(rest_data[0]) + 2  # +2 for '\r\n'
                                    complete_data = buffer[:end_index]
                                    # デバッグの邪魔なので止めておく - kotani (2025/11/26)
                                    #print(f"Received complete data from {device_name}: {complete_data}")
                                    
                                    # TrackingRecordRequest の特別処理
                                    if request_type == "TrackingRecordRequest":
                                        response_text = complete_data.decode().upper()
                                        if "MEMORYFULL" in response_text or "NG" in response_text:
                                            print(f"ERROR: {device_name} returned error for TrackingRecordRequest: {response_text}")
                                            # エラー応答をフロントエンドに送信
                                            await websocket.send(f"{device_name}:ERROR:{response_text}")
                                            # エラー時はDataRequestに切り替えて継続
                                            print(f"Switching to DataRequest for {device_name} due to TrackingRecordRequest error")
                                            return await fetch_device_data(device, websocket, "DataRequest")
                                        else:
                                            # TrackingRecordRequestの正常応答時は301番形式で応答
                                            tracking_response = generate_tracking_record_response(device_name)
                                            await websocket.send(f"{device_name}:{tracking_response}")
                                            print(f"Generated 301 tracking response for {device_name}: {tracking_response[:100]}...")
                                    else:
                                        # DataRequest の場合は拡張レスポンスを生成
                                        raw_response = complete_data.decode()
                                        enhanced_response = enhance_device_response(raw_response, device_name)
                                        
                                        # 現在の測定値を保存
                                        DeviceSettingsManager.update_current_data(device_name, raw_response)
                                        
                                        await websocket.send(f"{device_name}:{enhanced_response}")
                                        # デバッグの邪魔なので止めておく - kotani (2025/11/26)
                                        #print(f"Enhanced response for {device_name}: {len(enhanced_response.split())} data items")
                                    
                                    buffer = buffer[end_index:]  # 残りのデータを保持
                                    break

                except socket.timeout:
                    if buffer:
                        print(f"Received data before timeout from {device_name}: {buffer}")
                        # TrackingRecordRequest のタイムアウト時はDataRequestに切り替え
                        if request_type == "TrackingRecordRequest":
                            print(f"TrackingRecordRequest timeout for {device_name}, switching to DataRequest")
                            return await fetch_device_data(device, websocket, "DataRequest")
                        # DataRequestの場合は拡張レスポンスを生成
            ...
            print(f"WebSocket connection closed for device {device_name}: {e}")
            return  # WebSocket接続が閉じられた場合は関数を終了
    ...
    while True:
        try:
            ...
            if "devices" in data:
                ...
                # 新しいタスクを作成
                # デバッグの邪魔なので止めておく - kotani (2025/10/29)
                ...
                # 既存のTrackingRecordRequestなどの処理
                elif device_name in devices:
                    print(f"tracking:{device_name}")
